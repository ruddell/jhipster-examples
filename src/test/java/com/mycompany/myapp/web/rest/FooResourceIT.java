...
import com.mycompany.myapp.EsApp;
...
public class FooResourceIT {
     ...
     *
     * @see com.mycompany.myapp.repository.search.FooSearchRepositoryMockConfiguration
     */
    @Autowired
    private FooSearchRepository mockFooSearchRepository;
    ...
    public void createFoo() throws Exception {
        ...
        // Validate the Foo in Elasticsearch
        verify(mockFooSearchRepository, times(1)).save(testFoo);
    }
    ...
    public void createFooWithExistingId() throws Exception {
        ...
        // Validate the Foo in Elasticsearch
        verify(mockFooSearchRepository, times(0)).save(foo);
    }
    ...
    public void deleteFoo() throws Exception {
        ...
        // Validate the Foo in Elasticsearch
        verify(mockFooSearchRepository, times(1)).deleteById(foo.getId());
    }

    @Test
    @Transactional
    public void searchFoo() throws Exception {
        // Initialize the database
        fooRepository.saveAndFlush(foo);
        when(mockFooSearchRepository.search(queryStringQuery("id:" + foo.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(foo), PageRequest.of(0, 1), 1));
        // Search the foo
        restFooMockMvc.perform(get("/api/_search/foos?query=id:" + foo.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(foo.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)));
