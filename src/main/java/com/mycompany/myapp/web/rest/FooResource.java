...
public class FooResource {
    ...
    /**
     * {@code PATCH  /foos} : Updates given fields of an existing foo.
     *
     * @param fooDTO the fooDTO to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated fooDTO,
     * or with status {@code 400 (Bad Request)} if the fooDTO is not valid,
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/foos", consumes = "application/merge-patch+json")
    public ResponseEntity<FooDTO> partialUpdateFoo(@RequestBody FooDTO fooDTO) throws URISyntaxException {
        log.debug("REST request to update Foo partially : {}", fooDTO);
        if (fooDTO.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }

        Optional<FooDTO> result = Optional.ofNullable(fooService.partialUpdate(fooDTO));
