...
import com.jhipsterpress2.web.service.InterestService;
...
import com.jhipsterpress2.web.service.InterestQueryService;
...
public class InterestResourceIntTest {
    ...
    @Test
    @Transactional
    public void getAllInterestsByPartyIsEqualToSomething() throws Exception {
        // Initialize the database
        Party party = PartyResourceIntTest.createEntity(em);
        em.persist(party);
        em.flush();
        interest.addParty(party);
        interestRepository.saveAndFlush(interest);
        Long partyId = party.getId();

        // Get all the interestList where party equals to partyId
        defaultInterestShouldBeFound("partyId.equals=" + partyId);

        // Get all the interestList where party equals to partyId + 1
        defaultInterestShouldNotBeFound("partyId.equals=" + (partyId + 1));
    }
